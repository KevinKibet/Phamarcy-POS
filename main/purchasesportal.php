...
$id=$_GET['iv'];
include('../connect.php');
$resultaz = $dbh->prepare("SELECT * FROM purchases WHERE invoice_number= :xzxz");
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<?php
				$id=$_GET['iv'];
				$result = $dbh->prepare("SELECT * FROM purchases_item WHERE invoice= :userid");
			...
			<td><?php
			$rrrrrrr=$row['name'];
			$resultss = $dbh->prepare("SELECT * FROM products WHERE product_code= :asas");
			...
			<tr>
				...
				$sdsd=$_GET['iv'];
				$resultas = $dbh->prepare("SELECT sum(cost) FROM purchases_item WHERE invoice= :a");
